...
exports.postReview = async (
  ...
  try {
    ...
    const checkDuplicateSql = `select id from review_Table 
            where user_id = ? and title = ? and type = ?`;
    const [existingReviews] = await connection.query(checkDuplicateSql, [
      userId,
      title,
      type,
    ]);
...
exports.putReview = async (
  ...
  try {
    ...
    const [updateResult] = await connection.query(updateReviewSql, [
      title,
      score,
      content,
      watch_date,
    ...
    await connection.query(`delete from reviewTag_Table where review_id = ?`, [
      reviewId,
    ]);
...
exports.deleteReview = async (reviewId, userId) => {
  ...
  try {
    ...
    const [result] = await connection.query(deleteReviewSql, [
      reviewId,
      userId,
    ]);
